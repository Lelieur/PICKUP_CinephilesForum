...
const ReviewCard = ({
  ...
  const handleLike = () => {
    ReviewServices.likeReview(reviewData._id, loggedUser)
      .then((response) => {
  ...
  const handleDislike = () => {
    ReviewServices.dislikeReview(reviewData._id, loggedUser)
      .then((response) => {
        const { data: reviewData } = response;
        setReviewData(reviewData);
      })
  ...
  const handleEdit = (e) => {
    ...
    ReviewServices.editReview(reviewData._id, reviewData)
      .then(() => {
  ...
  const handleReviewDelete = () => {
    ReviewServices.deleteReview(_id)
  ...
  return (
    <div className="ReviewCard">
      <Card className="p-3 m-0 mb-3 text-white text-start">
        <Row className="pt-1">
          <Col md={2} className="ms-2 position-relative d-none d-md-inline">
          ...
          <Col md={9}>
            ...
            <Row className="mt-3">
              <Col xs={2} md={2} xl={1} className="pe-0 d-none d-md-block">
                ...
                <Row>
                  <Col className="ps-md-0">
                    <p className="m-0 fs-6">
                      <span className="fw-bold">{author?.firstName}</span> @
          ...
          >
            {!reviewData.usersLikes?.includes(loggedUser?._id) && (
              <Button
                ...
                size="lg"
                onClick={() => {
                  handleLike(), setIsReviewLiked(true);
                }}
              >
                <Heart />
              </Button>
            ...
            {reviewData.usersLikes?.includes(loggedUser?._id) && (
              <Button
                ...
                size="lg"
                onClick={() => {
                  handleDislike(), setIsReviewLiked(false);
                }}
              >
                <HeartFill />
              </Button>
      ...
      >
        <Modal.Body className="ps-4 pe-4">
          <Form onSubmit={handleEdit}>
            <Row className="h-100">
              <Col md={{ span: 2 }} className="rounded p-0 position-relative">
                <img
            ...
            <Row>
              <Col className="text-end">
                <Button className="rounded-pill border-0" type="submit">
                  Editar Reseña
                </Button>
              </Col>
            </Row>
